...
{
    public class BulkAppointmentService : IBulkAppointmentService
    {
        private readonly ApplicationDbContext _db;
        private readonly IOptions<BulkProcessingOptions> _options;

        public BulkAppointmentService(ApplicationDbContext db, IOptions<BulkProcessingOptions> options)
        {
            _db = db;
            _options = options;
        }
        ...
        public async Task<(string JobId, string HangfireJobId)> EnqueueProcessingForFile(string filePath)
        {
            ...
            var hangfireJobId = BackgroundJob.Enqueue<BulkAppointmentProcessor>(p => p.ProcessFile(filePath, jobId));
